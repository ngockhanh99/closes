...
myApp.controller('slideCtrl', function ($scope, $http, $rootScope) {
    $scope.data = {
        listData: {},
        singleData: {},
        isInsert: true,
    ...
    $scope.actions = {
        ...
        getAll() {
            ...
            $http.get(SITE_ROOT + 'dochoi/slide', { params: $scope.search })
                .then(response => {
        ...
        insert() {
            $rootScope.reloadPage = true
            $http.post(SITE_ROOT + 'dochoi/slide/insert', $scope.data.singleData)
                .then(() => {
                    $scope.actions.getAll()
        ...
        update(id) {
            $rootScope.reloadPage = true
            $http.put(SITE_ROOT + 'dochoi/slide/update/' + id, $scope.data.singleData)
                .then(() => {
                    $scope.actions.getAll()
        ...
        delete(id) {
            ...
            if (!check) return
            $http.delete(SITE_ROOT + 'dochoi/slide/delete/' + id)
                .then(() => {
                    $scope.actions.getAll()
        ...
        showModal(status, index) {
            ...
            if (status) $scope.data.singleData.is_banner = 1
