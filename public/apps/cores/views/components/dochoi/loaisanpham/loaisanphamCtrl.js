...
myApp.controller('loaisanphamCtrl', function ($scope, $http, $rootScope) {
    $scope.data = {
        listData: {},
        singleData: {},
        isInsert: true,
    ...
    $scope.actions = {
        initial() {
            ...
            $scope.actions.getListDanhmuc()
            fileUpload("button_upload", "input_upload");
        },
        getAll() {
            $rootScope.reloadPage = true
            $http.get(SITE_ROOT + 'dochoi/loaisanpham', { params: $scope.search })
                .then(response => {
        ...
        getListDanhmuc() {
            $http.get(SITE_ROOT + 'dochoi/danhmuc')
                .then(response => {
                    $scope.data.listDanhmuc = response.data
                })
        },
        insert() {
            $rootScope.reloadPage = true
            $http.post(SITE_ROOT + 'dochoi/loaisanpham/insert', $scope.data.singleData)
                .then(() => {
                    $scope.actions.getAll()
        ...
        update(id) {
            $rootScope.reloadPage = true
            $http.put(SITE_ROOT + 'dochoi/loaisanpham/update/' + id, $scope.data.singleData)
                .then(() => {
                    $scope.actions.getAll()
        ...
        delete(id) {
            ...
            if (!check) return
            $http.delete(SITE_ROOT + 'dochoi/loaisanpham/delete/' + id)
                .then(() => {
                    $scope.actions.getAll()
