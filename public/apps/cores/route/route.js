app.config(function ($stateProvider, $locationProvider, $urlRouterProvider, $ocLazyLoadProvider) {
    ...
    $stateProvider
        ...
        .state('config-connect', {
            url: '/config-connect',
            controller: 'configConnectCtrl',
            templateUrl: SITE_ROOT + 'apps/cores/views/components/config/configConnect/configConnect.html',
            resolve: {
                loadMyCtrl: ['$ocLazyLoad', 'Auth', function ($ocLazyLoad, Auth) {
                    return Auth.checkLogin.then(() => {
                        return $ocLazyLoad.load([
                            SITE_ROOT + 'apps/cores/views/components/config/configConnect/configConnectCtrl.js',
                        ]);
                    });
                }]
            }
